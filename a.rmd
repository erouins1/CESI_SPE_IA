...
model_linear <- train(
  Weight ~ Maternal_age + Maternal_weight + Maternal_height + Parity + Sex + Gestational_age,
  data = train_set,
  method = "lm",
...
model_p_c <- train(
  log(Weight) ~ poly(Gestational_age, 2) + Maternal_age + Maternal_weight + Maternal_height + Parity + Sex,
  data = train_set,
  method = "lm",
  trControl = train_control
)
...
```{r}
model_poly <- train(
  Weight ~ poly(Maternal_age, 2) + poly(Maternal_weight, 2) +
...
for (i in 1:n_folds) {
  ...
  predictions <- predict(model_mixed, newdata = testing_data, re.form = NA)  # NA to ignore random effects
