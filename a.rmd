...
model_p_c <- lm(log(Weight) ~ poly(Gestational_age, 2) + Maternal_age + Maternal_weight + Maternal_height + Parity + Sex, data=train_set)
...
# Fit the mixed linear model on the training dataset
model_mixed <- lmer(Weight ~ Maternal_age + Maternal_weight + Maternal_height +
  Parity + Sex + Gestational_age + (1 | Group), data = train_set)

# Summary of the mixed model
summary(model_mixed)

# Evaluate the model on the testing dataset
predictions_mixed <- predict(model_mixed, newdata = test_set)
test_mse_mixed <- mean((test_set$Weight - predictions_mixed)^2)
print(test_mse_mixed)
...
simulated_data$Weight_Mixed <- predict(model_mixed, newdata = simulated_data)
...
```{r}
# simulated_data$Weight_Mixed <- predict(model_mixed, newdata = simulated_data)
# p_mixed <- plot_percentiles(simulated_data, "Weight_Mixed", "Mixed Linear Model: Growth Percentiles", "grey")
#
# comparison_data <- simulated_data %>%
#   group_by(Gestational_age) %>%
#   summarise(
#     Average_Weight_Mixed = mean(Weight_Mixed, na.rm = TRUE)  # adding average for mixed model
#   )
#
# # Plot graph with curves from all models
# p_comparison <- ggplot(comparison_data, aes(x = Gestational_age)) +
#   geom_line(aes(y = Average_Weight_Mixed, colour = "Mixed Linear Model"), size = 1.2) +
#   scale_colour_manual(values = c("Linear Model" = "blue", "Polynomial Model" = "red", "Polynomial C Model" = "black", "Mixed Linear Model" = "grey")) +
#   labs(title = "Comparison of Model Predictions", y = "Average Predicted Weight", x = "Gestational Age", colour = "Model Type")
#
# # Print plots
# print(p_mixed)
```
