...
all_files <- lapply(list.files(path = "./data", pattern = "*.csv", full.names = TRUE), read.csv)
dta <- do.call(path = "./data", pattern = "*.csv", all_files)
...
# Sauvegarder le data frame combiné dans un nouveau fichier CSV
write.csv(dta, "donnees_combinees.csv", row.names = FALSE)


```
```{r}
# Lire le nouveau fichier CSV
donnees_lues <- read.csv("donnees_combinees.csv")

# Afficher les premières lignes du nouveau data frame pour vérification
head(donnees_lues)
```

```{r}
data_d <- distinct(donnees_lues)
write.csv(data_d, "data.csv", row.names = FALSE)
```
